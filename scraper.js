...
async function scrapeMedia(url) {
  ...
  // ─── Fallback Instagram via nayan-media-downloader ───
  if (platform === "instagram") {
    try {
      console.log(`[Scraper] yt-dlp gagal, mencoba nayan-media-downloader untuk Instagram...`);
      const { ndown } = require('nayan-media-downloader');
      const result = await ndown(url);
      
      if (result && result.status && result.data && result.data.length > 0) {
        const mediaItems = result.data.map((item, index) => {
        ...
        console.log(`[Scraper] Berhasil via nayan-media-downloader (${mediaItems.length} item)`);
        return {
          platform: "instagram",
          type: mediaItems.length > 1 ? "playlist" : (mediaItems[0].type || "image"),
          ...
          duration: null,
          mediaItems: mediaItems,
          source: "nayan_scraper",
          warning: null
        };
      }
    } catch (err) {
      console.warn(`[Scraper] nayan-media-downloader gagal: ${err.message}`);
    }
  }
