...
function extractMediaItem(entry) {
  ...
  // Jika tidak ada format yang tersaring (misal gambar) tapi ada URL, kita jadikan default
  ...
  if (isVideo && !availableFormats.some(f => f.type === 'audio')) {
    availableFormats.push({
      ...
      ext: 'mp3' // Proxy akan menamakannya mp3, kebanyakan player bisa memutarnya meskipun isinya mp4
    });
  }

  // Ambil thumbnail terbaik: cek field langsung, lalu array thumbnails (Twitter/X pakai ini)
  let thumb = entry.thumbnail || null;
  if (!thumb && entry.thumbnails && entry.thumbnails.length > 0) {
    // Ambil thumbnail resolusi tertinggi (biasanya yang terakhir di array)
  ...
  // Fallback terakhir: gunakan URL video itu sendiri
...
async function scrapeMedia(url) {
  ...
  if (ytdlpAvailable) {
    try {
      ...
      // TikTok: coba retry dengan cookies browser
      if (platform === "tiktok") {
        try {
      ...
      // Facebook: langsung gunakan Siputzx fallback jika yt-dlp gagal
      // karena cookies sering gagal akibat enkripsi DPAPI
  ...
  // Fallback ke oEmbed (Instagram & TikTok)
  if (platform === "instagram") {
    try {
      return await scrapeViaOEmbed(url);
    } catch (err) {
      throw new Error(
        `Semua metode scraping gagal untuk Instagram. ` +
        `Pastikan yt-dlp terinstall (pip install yt-dlp) dan URL valid.\n` +
        `Detail: ${err.message}`
      );
    }
  }
